...
function buildDecisionAccountabilityProductionSemanticPrompt({evidence}={}){
  ...
  const systemText=[
    ...
    'consequenceScope must be only the supported reach: individual_task, team, function, site, organization. Never choose the highest plausible scope when ambiguous. If positive authority is supported but consequence scope is missing or ambiguous, return UNSUPPORTED rather than a partial SUPPORTED candidate.',
    ...
    'Responsibility continuity is continuity of the represented decision responsibility, not role tenure. Preserve exact, approximate, lower_bound, upper_bound or range semantics. For known exact/approximate/lower_bound/upper_bound use months for the scalar value and keep minimumMonths/maximumMonths null; for range use minimumMonths and maximumMonths and keep months null. Example: over one year = {state: known, qualification: lower_bound, months: 12, minimumMonths: null, maximumMonths: null}. Missing/indeterminate duration is unknown with all temporal values null and never zero.',
