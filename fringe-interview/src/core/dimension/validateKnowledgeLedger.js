...
function fp(ids){return crypto.createHash("sha256").update(["knowledge-ledger","1.0",...ids].join("|")).digest("hex");}
function validateKnowledgeLedger(ledger={}){
 ...
 items.forEach((c,i)=>{const v=validateDimensionContribution(c); if(!v.valid) errors.push(`contributions[${i}] is invalid: ${v.errors.join(" | ")}`); if(c&&validString(c.id)){if(ids.has(c.id)) errors.push(`Duplicate DimensionContribution id: ${c.id}.`); ids.add(c.id);} if(i>0){const p=items[i-1]; if(p&&c&&(p.metadata.createdAt>c.metadata.createdAt||(p.metadata.createdAt===c.metadata.createdAt&&p.id.localeCompare(c.id)>0))) errors.push("contributions must use canonical createdAt/id ordering.");}});
 const sortedIds=[...ids].sort(); const expected=`knowledgeLedger_${fp(sortedIds).slice(0,32)}`; if(ledger.id!==expected) errors.push("id does not match canonical Ledger identity.");
