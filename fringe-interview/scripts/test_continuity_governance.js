...
assert.equal(repositoryResult.plannedTask, "0100E-10");

function buildFixture({ roadmapRows, nextTask = "0100E-10", nextStatus = "PLANNED", continuityNextTask = "0100E-10", configurationState = "APPROVED", workflowText = "# Workflow\nContinuity Impact Assessment\n", readmeText = "| `CONTINUITY.md` | CURRENT | state |\n| `NEXT_PHASE.md` | CURRENT | next |\n" }) {
  ...
  fs.writeFileSync(
    ...
    `# Continuity\nVerified through: **Task 0100E-9**\nThe next planned task is \`${continuityNextTask}\`.\nKnowledgeAcquisitionCapabilityConfiguration: ${configurationState}, not implemented.\n`
  );
  fs.writeFileSync(
...
const nextPhaseMismatch = inspectFixture({ roadmapRows: validRows, nextTask: "0100E-11" });
...
assert.equal(implementedConfiguration.status, "FAIL");
assert(implementedConfiguration.errors.some((error) => error.includes("exactly one PLANNED next task")));
assert(implementedConfiguration.errors.some((error) => error.includes("must not be described as IMPLEMENTED")));
