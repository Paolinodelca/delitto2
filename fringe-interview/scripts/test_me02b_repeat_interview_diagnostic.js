...
assert(source.includes('recentQuestionKeys = []'),'MVP must accept existing recent question keys');
assert(source.includes('recentQuestionHistory = []'),'MVP must accept existing recent question history');
...
console.log(JSON.stringify({repeatInterview:'EXISTING_VARIATION_CAPABILITY_CONSUMED_BY_ME02C',deterministicWithoutHistory:true,recentHistoryCapability:true,topBaselineKeys:top},null,2));
