...
async function main() {
  const fixtureDir = resolveProjectPath("fixtures", "test_junior_ops");
  const outputDir = resolveProjectPath("tmp", "app-mvp-session");

  await mkdir(outputDir, { recursive: true });

  const cvText = await readTextFile(path.join(fixtureDir, "cv_junior_ops.txt"));
  const jdText = await readTextFile(
    path.join(fixtureDir, "role_junior_product_ops.txt")
  );
  const answersFixture = await readJsonFile(
    path.join(fixtureDir, "answers_junior_ops.json")
  );
  ...
  const result = await runFringeInterviewMVPSession({
    ...
    userNotes: "Test profile: junior candidate with limited operations/reporting experience.",
    roleNotes: "Junior Product Operations role. Do not evaluate as senior leadership profile.",
